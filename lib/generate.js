 ...
 */
class Generator {
  constructor(projectPath, apiKey, modelConfig = null) {
    this.projectPath = projectPath;
    this.indexPath = path.join(projectPath, 'embeddings-index');
    this.modelConfig = modelConfig;
    
    // Create embedding service with model config if available
    if (modelConfig) {
      this.embeddingService = new EmbeddingService(apiKey, modelConfig.vendor, modelConfig.model);
      this.metrics = new Metrics(modelConfig);
    } else {
