...
function checkRerankerRequirements(modelConfig) {
  ...
  const hasRerankerConfig = modelConfig['reranker-vendor'] && modelConfig['reranker-model'];
  ...
  const messages = [];
  const vendor = modelConfig['reranker-vendor'];
  ...
  const configValidation = validateRerankerConfig({
    vendor: modelConfig['reranker-vendor'],
