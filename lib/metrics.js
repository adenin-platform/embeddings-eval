...
class Metrics {
  ...
  addEvaluateMetrics(metrics) {
    ...
    const roundedCost = parseFloat(parseFloat(metrics.cost).toFixed(8));
    const roundedEmbeddingCost = parseFloat(parseFloat(metrics.embeddingCost || 0).toFixed(8));
    const roundedRerankerCost = parseFloat(parseFloat(metrics.rerankerCost || 0).toFixed(8));
    
    this.evaluateMetrics.push({
      search: metrics.search,
      tokens: metrics.tokens,
      rerankerTokens: metrics.rerankerTokens || 0,
      runtime: metrics.runtime,
      cost: roundedCost,
      embeddingCost: roundedEmbeddingCost,
      rerankerCost: roundedRerankerCost,
  ...
  /**
   * Add metrics for a reranker operation
   * @param {Object} metrics - Reranker metrics
   */
  addRerankerMetrics(metrics) {
    // Store cost as fixed decimal to prevent scientific notation
    const roundedCost = parseFloat(parseFloat(metrics.cost).toFixed(8));
    
    this.rerankerMetrics.push({
  ...
  getEvaluateTotals() {
    ...
    const totalRerankerTokens = this.evaluateMetrics.reduce((sum, m) => sum + (m.rerankerTokens || 0), 0);
    ...
    const totalEmbeddingCost = this.evaluateMetrics.reduce((sum, m) => sum + (m.embeddingCost || 0), 0);
    const totalRerankerCost = this.evaluateMetrics.reduce((sum, m) => sum + (m.rerankerCost || 0), 0);
    ...
    return {
      ...
      totalRuntime,
      totalCost: parseFloat(totalCost.toFixed(8)), // Round to 8 decimal places
      totalEmbeddingCost: parseFloat(totalEmbeddingCost.toFixed(8)),
      totalRerankerCost: parseFloat(totalRerankerCost.toFixed(8)),
  ...
  /**
   * Get reranker totals
   * @returns {Object} - Reranker total metrics
   */
  getRerankerTotals() {
    if (this.rerankerMetrics.length === 0) {
      return {
        queryCount: 0,
        totalDocuments: 0,
        totalRuntime: 0,
        ...
        averageRuntimePerQuery: 0,
        averageCostPerQuery: 0
      };
    }

    const totalRuntime = this.rerankerMetrics.reduce((sum, m) => sum + m.runtime, 0);
    const totalCost = this.rerankerMetrics.reduce((sum, m) => sum + m.cost, 0);
    const totalDocuments = this.rerankerMetrics.reduce((sum, m) => sum + m.documentsCount, 0);
    const queryCount = this.rerankerMetrics.length;
    ...
    return {
      ...
      averageDocumentsPerQuery: Math.round(totalDocuments / queryCount * 10) / 10,
      averageRuntimePerQuery: Math.round(totalRuntime / queryCount),
      averageCostPerQuery: parseFloat((totalCost / queryCount).toFixed(8))
    };
  }
  /**
  ...
  getAllMetrics() {
    ...
    // Include reranker metrics only if they exist
    if (this.rerankerMetrics.length > 0) {
      result.reranker = {
        items: this.rerankerMetrics,
        totals: this.getRerankerTotals()
      };
    }
