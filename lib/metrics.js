...
class Metrics {
  ...
  calculateCost(tokens) {
    if (!this.modelConfig || !this.modelConfig.cost) {
      return 0.1; // Fallback to original static cost
    }
    // Cost is per 1 million tokens, so divide by 1,000,000
    return (tokens * this.modelConfig.cost) / 1000000;
  }
  ...
  addGenerateMetrics(metrics) {
    const cost = this.calculateCost(metrics.tokens);
